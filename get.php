...
//68.9722 mile = 111 km = 1 lat/long point
  ...
  if(isset($_GET['lat']) && isset($_GET['long']) && isset($_GET['distance'])){
    ...
    $pdo = new PDO('mysql:host='.$db_host.';dbname='.$database, $username, $pwd);
       ...
       if($filters > 0){
         $query1 = $pdo->prepare("SELECT EventName, Lat, Lon, Description, StartTime, EndTime, Category, Image, Address, Likes, Hash FROM events WHERE Reports < 5 AND Lat >= ".$latLower." AND Lat <= ".$latUpper." AND Lon >= ".$longLower." AND Lon <= ".$longUpper." AND (Category = ".$filterQuery. ")");
       }
       else{
         $query1 = $pdo->prepare("SELECT EventName, Lat, Lon, Description, StartTime, EndTime, Category, Image, Address, Likes, Hash FROM events WHERE Reports < 5 AND Lat >= ".$latLower." AND Lat <= ".$latUpper." AND Lon >= ".$longLower." AND Lon <= ".$longUpper);
       }
       //Execute query
